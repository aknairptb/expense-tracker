...
let settings = JSON.parse(localStorage.getItem(SETTINGS_KEY) || '{"budget":0,"currency":"₹"}');
...
function setupSettings() {
  ...
  document.getElementById('btn-clear-data').addEventListener('click', () => {
    showConfirm('Clear All Data', 'Are you sure you want to delete ALL expenses? This cannot be undone.', () => {
      expenses = [];
      save();
      modal.classList.remove('active');
...
function saveEdit() {
  ...
  if (idx === -1) return;
  expenses[idx].amount = parseFloat(document.getElementById('edit-amount').value);
  ...
  expenses[idx].description = document.getElementById('edit-description').value || expenses[idx].category;
  save();
  document.getElementById('edit-modal').classList.remove('active');
...
function importCSV(e) {
  ...
  reader.onload = function(ev) {
    try {
      ...
      save();
      showToast(`${imported} expenses imported!`);
      renderAll();
    } catch (err) {
